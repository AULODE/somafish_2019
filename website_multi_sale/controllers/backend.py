...
class WebsiteSaleBackend(WebsiteSaleBackend):
    ...
    def fetch_dashboard_data(self, date_from, date_to, website_id=None):
        ...
        sales_values = dict(
            ...
            summary=dict(
                ...
                order_per_day_ratio=0, order_sold_ratio=0,
                order_convertion_pctg=0,
            )
        )
        results['dashboards']['sales'] = sales_values

        results['groups']['sale_salesman'] = request.env[
            'res.users'].has_group('sales_team.group_sale_salesman')
        ...
        sale_order_domain = [
            ('team_id', 'in', request.env['crm.team'].search(
                [('team_type', '=', 'website')]).ids),
            ('date_order', '>=', fields.Datetime.to_string(datetime_from)),
            ('date_order', '<=', fields.Datetime.to_string(datetime_to)),
            ('website_id', '=', website_id)]
        so_group_data = request.env['sale.order'].read_group(sale_order_domain,
                                                             fields=['state'],
                                                             groupby='state')
        for res in so_group_data:
            if res.get('state') == 'sent':
                sales_values['summary']['order_unpaid_count'] += res[
                    'state_count']
        ...
        sales_values['summary'].update(
            ...
            payment_to_capture_count=request.env[
                ...
                ('sale_order_id.id', 'in', request.env['sale.order'].search(
                    sale_order_domain + [('state', '!=', 'cancel')]).ids),
            ]),
            total_sold=sum(price_line['price_subtotal'] for price_line in
                           report_price_lines)
        )
        ...
        sales_values['summary']['order_sold_ratio'] = round(
            ...
            sales_values['summary']['order_count'], 2) if \
        sales_values['summary']['order_count'] else 0
        sales_values['summary']['order_convertion_pctg'] = 100.0 * \
                                                           sales_values[
                                                               'summary'][
                                                               'order_count'] / \
                                                           sales_values[
                                                               'summary'][
                                                               'order_carts_count'] if \
        ...
        sales_values['graph'] += [{
            'values': self._compute_sale_graph(date_date_from, date_date_to,
                                               sales_domain),
            'key': 'Untaxed Total',
        }, {
            'values': self._compute_sale_graph(
                date_date_from - timedelta(days=date_diff_days),
                date_date_from, sales_domain, previous=True),
