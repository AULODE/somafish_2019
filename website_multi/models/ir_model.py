...
class IrModelData(models.Model):
    ...
    def _update(self, model, module, values, xml_id=False, store=True,
        ...
        # records created during module install should not display the
        # messages of OpenChatter
        self = self.with_context(install_mode=True)
        current_module = module

        if xml_id and ('.' in xml_id):
            assert len(xml_id.split('.')) == 2, _(
                "'%s' contains too many dots. XML ids should not contain "
                "dots ! These are used to refer to other modules data, "
